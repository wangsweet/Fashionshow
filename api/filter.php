...
function main(){
    ...
    if($connection->error){//判断链接是否成功
        ...
        if($rs=$connection->query("select * from products where ".$condition.";")){//$connection->query("select * from students;") 返回记录集
            //query方法可用于查询更新数据库信息
            echo "{code:2000,list:[";
            while($row=$rs->fetch_assoc()){//循环遍历数据库中表内的数据，每次循环读出一条数据并赋值给$row变量，一条信息就是表中的一行信息
                echo "{ID:".$row["id"].",name:'".$row["p_name"]."',price:'".$row["p_price"]."',color:'".$row["p_color"]."',brand:'".$row["p_brand"]."',sort:'".$row["p_sort"]."',image01:'".$row["p_image01"]."',image02:'".$row["p_image02"]."',image03:'".$row["p_image03"]."',image04:'".$row["p_image04"]."',size:'".$row["p_size"]."'},";
            }
            echo "]}";
        }
